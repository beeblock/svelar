...
- Use both FormRequest classes and DTOs for write paths. FormRequest owns validation and authorization with shared Zod or Valibot schema rules; DTOs carry validated data into services/actions. Do not pass raw `z.infer`, `v.InferOutput`, or unvalidated `FormData` directly into services.
- New apps choose their validation provider with `npx svelar new --validation=zod` or `--validation=valibot`; the choice is stored in `svelar.validation.json`. Generators must honor that file and avoid mixing schema styles in the same app.
