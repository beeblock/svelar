...
Use `HasUuids` or `HasUlids` when the primary key itself should be generated by the model:

```typescript
import { HasUlids, HasUuids, Model } from '@beeblock/svelar/orm';

export class Order extends HasUuids(Model) {
  static table = 'orders';
}

export class Invoice extends HasUlids(Model) {
  static table = 'invoices';
}
```

Pair those models with `table.uuid('id').primary()` or `table.ulid('id').primary()` in the migration.

For APIs, the recommended default is still an integer primary key plus a public UUID/ULID column:

```typescript
export class Customer extends Model {
  static table = 'customers';
  static uniqueIds = ['uuid'];
  static uniqueIdType = 'uuid' as const; // UUID v7
  static hidden = ['id'];
}
```
...
Use `static uniqueIdType = 'ulid' as const` with `table.ulid('ulid').unique()` when you prefer ULIDs.
...
Use the `SoftDeletes` mixin when deletes should mark `deleted_at` instead of removing the row:
...
Soft-deleted rows are excluded by default. Use the explicit scopes when you need them:

```typescript
await post.delete();                    // sets deleted_at
await Post.withTrashed().find(post.id);  // includes deleted rows
await Post.onlyTrashed().get();          // only deleted rows
await post.restore();                    // clears deleted_at
await post.forceDelete();                // removes the row
```
...
Add Laravel-style soft deletes to a model:
...
Generate UUID v7 or ULID primary keys before inserting the model:

```typescript
import { HasUlids, HasUuids, Model } from '@beeblock/svelar/orm';

class Order extends HasUuids(Model) {
  static table = 'orders';
}

class Invoice extends HasUlids(Model) {
  static table = 'invoices';
}
```

For public secondary IDs, keep the integer primary key and configure the generated columns directly:
