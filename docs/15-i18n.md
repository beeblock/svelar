...
> **i18n is opt-in.** New projects scaffolded with `npx @beeblock/svelar new` ship in English with no i18n overhead. When you're ready to add multi-language support, follow the steps below — Svelar's Vite aliases for `@beeblock/svelar/i18n` are already pre-configured.
...
Add the paraglide plugin to your existing `vite.config.ts` (the file is already scaffolded by `npx @beeblock/svelar new`):
