...
export class MakeRepositoryCommand extends Command {
  ...
  async handle(args: string[], flags: Record<string, any>): Promise<void> {
    ...
    const reposDir = join(process.cwd(), 'src', 'lib', 'repositories');
    mkdirSync(reposDir, { recursive: true });

    const filePath = join(reposDir, `${repoName}.ts`);
    ...
    const content = `import { Repository } from 'svelar/repositories';
...
`;
    ...
    this.success(`Repository created: src/lib/repositories/${repoName}.ts`);
  }
