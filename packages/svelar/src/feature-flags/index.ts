 ...
 * rollout support. Database tables are created by Svelar migrations.
 *
 * Usage:
...
class FeatureFlagManager {
  ...
  private get flagsTable(): string {
    return assertSqlIdentifier(this.config.table || 'feature_flags', 'Feature flags table name');
  }

  private get overridesTable(): string {
    return assertSqlIdentifier(this.config.overridesTable || 'feature_flag_overrides', 'Feature flag overrides table name');
  }
  ...
  private flagsQuery(): QueryBuilder<any> {
    return new QueryBuilder(this.flagsTable);
  }

  private overridesQuery(): QueryBuilder<any> {
    return new QueryBuilder(this.overridesTable);
  }
  ...
  async getFlag(name: string): Promise<FeatureFlag | null> {
    if (this.useDb) {
      const row = await this.flagsQuery().where('name', name).first();
  ...
  async allFlags(): Promise<FeatureFlag[]> {
    if (this.useDb) {
      const rows = await this.flagsQuery().orderBy('name').get();
  ...
  async deleteFlag(name: string): Promise<boolean> {
    if (this.useDb) {
      await this.overridesQuery().where('flag_name', name).delete();
      const deleted = await this.flagsQuery().where('name', name).delete();
      return deleted > 0;
    }
  ...
  async setOverride(
    ...
    if (this.useDb) {
      const nowIso = new Date(now).toISOString();
      await this.overridesQuery().upsert(
        {
          id: randomUUID(),
          flag_name: flagName,
  ...
  async removeOverride(
    ...
    if (this.useDb) {
      const deleted = await this.overridesQuery()
  ...
  async getOverride(
    ...
    if (this.useDb) {
      const row = await this.overridesQuery()
        .where('flag_name', flagName)
        .where('scope_type', scopeType)
        .where('scope_id', String(scopeId))
        .first();
  ...
  async getOverrides(flagName: string): Promise<FeatureFlagOverride[]> {
    if (this.useDb) {
      const rows = await this.overridesQuery().where('flag_name', flagName).get();
